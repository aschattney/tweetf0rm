...
from proxies import proxy_checker
...
class Scheduler(object):
    def __init__(self, node_id, config={}, proxies=[]):
        ...
        if proxies and len(proxies) > 0:
            ...
            # each process only get one apikey...
            # if there are more proxies than apikeys,
            #  each process can get more than one proxy that can be rotated when one fails.
            number_of_processes = min(len(self.config['apikeys']), len(self.proxy_list))

            # if there are more proxies than apikeys, then each process will get a list of proxies
            #  and the process will restart it self if a proxy failed, and try the next available proxy
        ...
        for crawler_id in self.crawlers:
            ...
            if not cc['crawler'].is_alive():
                ...
                if 'retry_timer_start_ts' in cc and (time.time() - cc['retry_timer_start_ts'] > 1800):
                    # retry 30 mins after the crawler dies...
                    # mostly the crawler died because "Twitter API returned a 503 (Service Unavailable), Over capacity"
